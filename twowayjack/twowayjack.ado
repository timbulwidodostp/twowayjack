...
/*version 1 2024-06-11 */
/*------------------------------------*/
version 13

/*----------------------------------*/

/*two-way jack program */

*this makes three calls to summclust
*uses the max operator
*reports cv1 and cv3 multi-way standard errors
/*----------------------------------*/

cap program drop twowayjack
program define twowayjack, rclass

  syntax varlist(min=3), cluster(varlist) [fevar(varlist) SAMple(string)]
  
  local XTEMP = word("`varlist'",2)
  
  /*check if two-way?*/
  local numdims = wordcount("`cluster'")
  else if `numdims' > 2 {
    disp as error "twowayjack only supports two-way clustering at the moment."
    exit 198
  }
  
  else if `numdims' == 2 {
    
    local clust1  = word("`cluster'",1)
  
    local clust2  = word("`cluster'",2)
    
  }
  
  else if `numdims' == 1{
    disp as error "For a single dimension of clustering use summclust"
    exit 198
  }
  
  /*add if to sample*/
  if "`sample'" != "" {
      local SMPL = "sample(`sample')"
  }

  /*matrix to store output*/
  *summtable
  mata: summtable = J(3,6,.)
  mata: numclust = J(1,3,.)
    
  *first dimension
   cap quietly mwsummclust `varlist', cluster("`clust1'") fevar(`fevar') ///
   `SMPL' nog gstar
   
   mata: cv1g = cv1se*cv1se
   mata: cv3g = cv3se*cv3se
   
   mata: summtable[1,1::4] = clustsum[7,1::4]
   mata: summtable[1,5] = rows(betag)
   mata: summtable[1,6] = gstarzero
   
   mata: numclust[1,1] = rows(betag)
  
  *second dimension
   cap quietly mwsummclust `varlist', cluster("`clust2'") fevar(`fevar') ///
   `SMPL' nog gstar
      
   mata: cv1h = cv1se*cv1se
   mata: cv3h = cv3se*cv3se
   
   mata: summtable[2,1::4] = clustsum[7,1::4]
   mata: summtable[2,5] = rows(betag)
   mata: summtable[2,6] = gstarzero
   
   mata: numclust[1,2] = rows(betag)
  
  *intersection
  
  tempvar ints 
  
  egen `ints'  = group(`clust1' `clust2')
  
  cap quietly mwsummclust `varlist', cluster(`ints') fevar(`fevar') ///
  `SMPL' nog gstar
  
   mata: cv1i = cv1se*cv1se
   mata: cv3i = cv3se*cv3se
   
   mata: summtable[3,1::4] = clustsum[7,1::4]
   mata: summtable[3,5] = rows(betag)
   mata: summtable[3,6] = gstarzero
   
   mata: numclust[1,3] = rows(betag)

  *max 
  
  mata: cv1trad = cv1g + cv1h - cv1i
  mata: cv3trad = cv3g + cv3h - cv3i
  
  mata: cv1set= (cv1trad,cv1g,cv1h)
  mata: cv3set= (cv3trad,cv3g,cv3h)

  
  mata: cv1max = sqrt(max(cv1set))
  *mata: cv1set
  *mata cv1max
  
  mata: cv3max = sqrt(max(cv3set))
  *mata: cv3set
  *mata cv3max
  
  mata: dfmax = min(numclust)

  *regression output
  *regoutput
  mata: mwcvstuff = J(2,6,.)
  
  mata mwcvstuff[1,1] = cvstuff[1,1]
  mata mwcvstuff[1,2] = cv1max
  mata mwcvstuff[1,3] = cvstuff[1,1] / cv1max
  mata mwcvstuff[1,4] = ///
  2*min((ttail(dfmax-1,mwcvstuff[1,3]),1-ttail(dfmax-1,mwcvstuff[1,3])))
  mata mwcvstuff[1,5] = cvstuff[1,1] - invttail(rows(betag)-1,0.025) * cv1max
  mata mwcvstuff[1,6] = cvstuff[1,1] + invttail(rows(betag)-1,0.025) * cv1max
  
  mata mwcvstuff[2,1] = cvstuff[1,1]
  mata mwcvstuff[2,2] = cv3max
  mata mwcvstuff[2,3] = cvstuff[1,1] / cv3max
  mata mwcvstuff[2,4] = ///
  2*min((ttail(dfmax-1,mwcvstuff[2,3]),1-ttail(dfmax-1,mwcvstuff[2,3])))
  mata mwcvstuff[2,5] = cvstuff[1,1] - invttail(dfmax-1,0.025) * cv3max
  mata mwcvstuff[2,6] = cvstuff[1,1] + invttail(dfmax-1,0.025) * cv3max

  mata: st_matrix("mwcvstuff", mwcvstuff)

  *report results
  
  *matrix list mwcvstuff
        
  /*output display*/
  disp ""
  disp ""
  disp "TWOWAYJACK"
  disp "Reference: James G. MacKinnon, Morten Ø. Nielsen, and Matthew D. Webb"
  disp "           Jackknife inference with two-way clustering"
  disp " "
  disp "Two-way cluster jackknife variance estimation."
  disp "Cluster summary statistics for `XTEMP' when clustered by `clust1' and `clust2'."
  *disp "There are `numobs' observations within `G' `cluster' clusters."
    
  local RSPECCV "&|&|"
  matrix colnames mwcvstuff = "Coeff" "Sd. Err." "t-stat" "P value" ///
  CI-lower CI-upper
  matrix rownames mwcvstuff = "CV1max" "CV3max"
      
  matlist mwcvstuff, title(Regression Output) rowtitle(s.e.) ///
  cspec(& %-4s w8 | %9.6f w10 & %9.6f & %6.4f w7 & %6.4f w7 & %9.6f w10 ///
  & %9.6f w10 &) rspec("`RSPECCV'")
    
  mata: st_matrix("summtable",summtable)
    
  matrix colnames summtable = Ng Leverage "Partial L." "beta no g" "G" "Gstar" 
  matrix rownames summtable = "`clust1'" "`clust2'" "intersect"
  matlist summtable, title(Coefficients of Variation, G, and G*) ///
  rowtitle(dimension) cspec(& %-4s w10 | %7.4f w8 & %7.4f w9 & %7.4f w10 ///
  & %7.4f w10 & %6.0f w7 & %8.2f w9 &) rspec("&|&&|")
...
cap program drop mwsummclust
program define mwsummclust, rclass
